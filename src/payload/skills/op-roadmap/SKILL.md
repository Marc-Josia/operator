...
become one op-new intake and travel a lane. Keep them vertical and independently shippable where
possible. You are naming the issues, not scoping them in detail; each gets its real triage, scope,
and (for standard/full) spec when op-new and op-plan pick it up. Note hard dependencies between
items in Sequencing.
...
approval — this is the project-level human gate, the analogue of op-plan's spec approval. Journal it
in the roadmap:
...
Take the first work item of the active milestone and route it to `.agents/skills/op-new/SKILL.md`.
