...
description: "Expertise pack for the scientific method on bugs: reproduce deterministically, isolate to a minimal repro, test one falsifiable hypothesis at a time, verify the root cause with evidence BEFORE any fix, and fix the mechanism, never the symptom. Consult it the moment a failure resists your first explanation — an unexplained test failure, intermittent behavior, or a fix that 'works' for reasons you cannot state; op-fix consumes it at its root-cause step, op-build when a mid-build failure resists quick diagnosis. Advisory only: it returns a verified root-cause finding and never moves work-item state."
---
